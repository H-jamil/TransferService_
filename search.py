...
def fixed_cc(configurations, black_box_function, logger, verbose=True):
    max_thread, count = configurations["thread_limit"], 0
    values = []
    ccs = [configurations["fixed_probing"]["thread"]]
    print("ccs={}".format(ccs))
    while True:
        count +=1
        values.append(run_probe(ccs[-1], count, verbose, logger, black_box_function))
        if values[-1] == 10 ** 10:
            logger.info("Optimizer Exits ...")
            break
        # logger.info("fixed cc step: {0} with cc {}".format(count, ccs[-1]))
